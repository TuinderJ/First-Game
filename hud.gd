...
func set_active_hotabr_slot(inventory_data: InventoryData, desired_active_hotbar_slot: int) -> void:
	hotbar.update_active_hotbar_slot(inventory_data, desired_active_hotbar_slot)

func increase_item_quantity(inventory_data: InventoryData, item_data: ItemData, amount_to_increase: int = 1) -> void:
